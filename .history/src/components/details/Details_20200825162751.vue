                ...
                <div v-for="(item,index) in menu" :key="index">
                  <div class="classifications">
                    <div class="m-l-10 d-f">
                      <div class="m-l-10 hov" @click="click(item)">{{item.child[0].title}}</div>
                      <div class="menu1"></div>
                    </div>
...
export default {
  ...
  methods: {
    click(item){
      this.values=item.child[0].title
      this.getData()
    },
    getData(){
      this.$api
        .products({ city: this.positionss, keyword: this.values })
        .then((res) => {
  ...
  mounted() {
    ...
    let positions=city.split("市")
    // console.log(positions);
    this.positionss=positions[0]
    // console.log(this.positionss);
    this.$api
