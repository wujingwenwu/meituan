            ...
            <div class="historys1" v-if="values.length !==0 && flag===true &&value ===''">
              ...
              <div class="d-f">
                <div v-for="(item,index) in values" :key="index">
                  <div class="f-s-12 m-t-10 m-l-10 hov2" >{{item}}</div>
                </div>
              </div>
            </div>
          </div>
...
　　　　　　type:String,
  ...
  methods: {
    ...
    //存localStorage
    change(value) {
      this.$router.push({name:"Detail",query:{value:value,username:this.username,positions:this.positions}})
      if (value !== "") {
        this.values.push(value);
       
        this.values = this.values.filter((item,index)=>{
          return this.values.indexOf(item) === index
        })
        localStorage.setItem("item", JSON.stringify(this.values));
        localStorage.setItem('positions',this.positions)
      }
    },
    clciker(){
